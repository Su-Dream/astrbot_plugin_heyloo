...
海络云 AstrBot 运营查询插件，用于通过聊天指令查询指定短链请求数、成功数、昨日点击总览和当前队列指标。

## 功能

- 支持 `/点击查询 <URL> <起始日期> <截止日期>` 指令，例如 `/点击查询 ln.run/9dEX9 2026-07-01 2026-07-02`
- 支持 `/昨日点击 <URL>` 指令，自动查询昨天 0 点到今天 0 点之间的请求数和成功数
...
```text
ln.run/9dEX9 在 2026-07-01 00:00:00 到 2026-07-02 00:00:00 内有 82 个请求，成功 87 个
```
...
- `queue_api_base_url`：队列服务器地址，例如 `http://127.0.0.1:8991`。插件会自动请求 `/queue-metrics`。
